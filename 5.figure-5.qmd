...
I'm pleased with this as a function, but better to loop through multiple heatmaps to include species etc.

```{r ggtree-Multiple-Heatmaps-Function,fig.height=14,fig.width=14}

# Function to plot a tree with multiple heatmaps
...
# Combine specific arguments with common arguments into a list
gheatmap_args <- c(
  list(plot = plot, data = extra_heatmap$df, offset = extra_heatmap$offset, width = extra_heatmap$width),
  common_args
)

# Call gheatmap with the combined arguments using do.call
gheatmap_plot <- do.call(gheatmap, gheatmap_args)

# Alternatively

# Define a wrapper function
my_gheatmap <- function(plot, data, offset, width, common_args) {
  do.call(gheatmap, c(
    list(plot = plot, data = data, offset = offset, width = width),
    common_args
  ))
}

# Call the wrapper function
gheatmap_plot <- my_gheatmap(plot, extra_heatmap$df, extra_heatmap$offset, extra_heatmap$width, common_args)
```



```{r Candida-SIT1-RAXML-Annotated,fig.height=14,fig.width=14}
require(scales)
print(pal_lancet()(7))
print(pal_viridis()(6))
