  ...
  #wide_mat$Condition <- factor(wide_mat$Condition,
   #                                               levels=c("I","II","III","IV","V","Water","None"))
    mat <- as.matrix(wide_mat_simple[, -(1:3)])
     library(ggfortify)
    pca_res <- prcomp(mat, scale. = TRUE)
  title = expression(paste("PCA Plot for FPKM in... many isolates (",italic("n")," = 572, 36 studies)"))
  library(pals)
plot <- autoplot(pca_res, data=wide_mat_simple, colour = "Theme") + labs(title=title) + theme_minimal() + scale_colour_manual(values=as.vector(alphabet(26)))
...
```{r}
``{r AK-enrichment,fig.height=10,fig.width=10}
