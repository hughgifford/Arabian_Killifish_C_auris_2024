...
p1 <- df_mtl_fpkm_narrow_plot %>% mutate(Cluster=gsub("uniq_.*","Other",Cluster)) %>% 
  ...
  ggplot(aes(x=Start,xend=Stop,y=FPKM,yend=FPKM, colour=Figure_name)) + geom_segment() + facet_wrap(Condition~Clade,ncol=5,scales = "free_x") + theme_minimal()+ theme(axis.text.x = element_blank(),axis.ticks.x = element_blank(),axis.line.x = element_blank()) +   #coord_cartesian(ylim = c(0, 300)) + 
...
```{r MTL-narrow-B8441}
df_mtl_fpkm_narrow_plot <- df_fpkm_ %>% select(-c(Clade,Reference_genome)) %>% left_join(.,df_mtl_narrow_plot,by="Locus_id") %>% 
  filter(!is.na(Clade))
...
```{r Cluster-FPKM-Check, fig.height=9,fig.width=7}
# Get genes to check
...
# Compare means
v <- df_fpkm_full_info_v %>% filter(Set=="Virulent") #%>% filter(CGD_name=="SCF1_1458") 
require(rstatix)
v_stat <- compare_means(data=v,FPKM ~ Status,group.by = "CGD_name",p.adjust.method = "bonferroni") %>%  mutate(y.position = max(v$FPKM, na.rm = TRUE) + 100) %>% p_adj_signif()# Bonferroni



v_plot <- df_fpkm_full_info_v %>% ggplot(aes(x=Status,y=FPKM,colour=Clade)) + geom_point() + theme_prism() + scale_color_nejm() + facet_wrap(~CGD_name,drop = T,ncol=1,scales="free_y") + stat_pvalue_manual(v_stat,label = "p.adj.signif", tip.length = 0.01,
  coord.flip = FALSE,vjust=1) + coord_flip()
v_plot
#ggsave(v_plot,file="~/Dropbox/phd/writing/auris_killifish/figures/dotplot_specific_checks_virulence_v0_2024_09_23.pdf",dev=cairo_pdf(),height = 9,width=7)
```
...
df_fpkm_full_info_f <- df_fpkm_specific_clusters %>% 
  mutate(Status=ifelse(Set=="Virulent" & (Clade=="I" | Clade =="IV"),"More Virulent",
                ifelse(Set=="Virulent" & (Clade=="II" | Clade == "III"), "Less Virulent",
                ifelse(Set=="Filament" & Clade =="V","Filamentous","Afilamentous")))) %>% filter(Set=="Filament")  
...
f <- df_fpkm_full_info_f_noHP %>% filter(Set=="Filament") #%>% filter(CGD_name=="SCF1_1458") 

f_stat <- compare_means(data=f,FPKM ~ Status,group.by = "CGD_name",p.adjust.method = "bonferroni") %>%  mutate(y.position = max(f$FPKM, na.rm = TRUE) + 100) %>% p_adj_signif()# Bonferroni

f_plot <- df_fpkm_full_info_f_noHP %>% ggplot(aes(x=Status,y=FPKM,colour=Clade)) + geom_point() + theme_prism() + scale_color_nejm() + facet_wrap(~CGD_name,drop = T,ncol=4,scales="free_y") + stat_pvalue_manual(f_stat,label = "p.adj.signif", tip.length = 0.01,
  coord.flip = FALSE,vjust=1) + coord_flip()
f_plot
#ggsave(f_plot,file="~/Dropbox/phd/writing/auris_killifish/figures/dotplot_specific_checks_filament_v0_2024_09_23.pdf",dev=cairo_pdf(),height = 11,width=21)
```
